...
<article class="blogs">
    <h1 class="t_nav"><span>分享你和我。</span><a href="/" class="n1">网站首页</a><a href="/" class="n2">分享区</a></h1>
    <div class="newblog left">

        <c:forEach items="${list}" var="list">
            <div class="line"></div>
            <h2>${list.shareHeadline}</h2>
            <p class="dateview"><span>发布时间：${list.shareTime}</span><span>作者： ${list.user.nickname}</span><span>分类：[<a href="/news/life/">邻里分享</a>]</span></p>
            <c:forEach items="${list.sharePictures}" var="pic">
                <figure><img src="share/doshow?sharePictureName=${pic.sharePictureAttribute}" width="175" height="117" id="allUrl"></figure>
            </c:forEach>
            <ul class="nlist">
                <p>${fn:substring(list.shareContent,0,80) }${fn:substring(".........",0,10)}</p>
                <a title="/" href="share/findId/${list.shareId}" target="_self" class="readmore">阅读全文>></a>
            </ul>
        </c:forEach>
        <div class="line"></div>
        <div class="blank"></div>
        <div class="ad">
            ...
            <div class="mail">
                <div class="send">
                    <div class="sendbox">
                        <form  action="share/add" id="jvForm" method="post" class="messages" enctype="multipart/form-data">

                            <div class="messlist" >
                                <label>标题</label>
                                <input type="text" name="shareHeadline">
                            </div>

                            <div class="messlist textareas">
                                <label>帖子内容</label>
                                <textarea type="text" placeholder="说点什么吧..." name="shareContent"></textarea>
                                <div class="clears"></div>
                            </div>
                            <label>添加图片</label> <input type="file" name="file" id="sharePictureName" >
        ...
        <div class="d1">
            一共${page.pages}页
        <%--<p>${page.pageNum }</p>--%>
            <a href="share/list?page=${page.firstPage}">第一页</a>
            <a href="share/list?page=${page.prePage}">上一页</a>
            <c:forEach var='i' begin="1" end="${page.pages }" step="1">
                <a href="share/list?page=${i }">${i }</a>
            </c:forEach>
            <a href="share/list?page=${page.nextPage}">下一页</a>
            <a href="share/list?page=${page.lastPage}">最后页</a>
        </div>
        </div>
