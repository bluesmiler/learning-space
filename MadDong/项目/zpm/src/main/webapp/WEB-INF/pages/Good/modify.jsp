...
<div class="container">
    ...
    <form class="form-horizontal col-xs-offset-1" action="good/modify" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <div class="col-xs-4">
                <label >商品类型
                    ...
                    <select id="type" name="goodType.id" class="form-control input-sm" >
                        <c:forEach items="${type}" var="ty">
                            <option value="${ty.id}" ${good.goodType.id == ty.id ? 'selected':''} >${ty.name}</option>
                            <%--<option value="${ty.id}">${ty.name}</option>--%>
                            <%--<c:when test="${ty.id==good.goodType.id}">--%>
